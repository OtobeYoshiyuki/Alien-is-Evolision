...
class HPGage:public OtobeLib::TextureBase
{
private:
	//画像の縦か横の最大値
	int m_Max;
...
public:
	//コンストラクタ
	HPGage(OtobeLib::ITextureBase* texture);
...
public:
	...
	//裏面のゲージを取得する
	ITextureBase* GetBackGageTexture() { return m_backTexture; }

	//イージング関数用の時間を設定する
	void SetEasingTime(const float& easing) { m_easingTime = easing; }
	
	//イージング関数用の時間を取得する
	const float& GetEasingTime()const { return m_easingTime; }

	//ゲージの生成
	void CreateGage(OtobeLib::TextureManager* textureManager);
};
