...
class Needle :public OtobeLib::TextureBase, OtobeLib::CollisionBase<OtobeLib::ITextureBase>
{
private:
	//入れ替え用のベクトル
	DirectX::SimpleMath::Vector3 m_TempVel;
public:
	//コンストラクタ
	Needle(OtobeLib::ITextureBase* texture);
	...
	//当たったかどうかのフラグを設定する
	void SetCheck(const bool& check)override { CollisionBase::SetCheck(check); }
...
public:
	//入れ替え用のベクトルを設定する
	void SetTempVel(const DirectX::SimpleMath::Vector3& vel) { m_TempVel = vel; }

	//入れ替え用のベクトルを取得する
	const DirectX::SimpleMath::Vector3& GetTempVel()const { return m_TempVel; }
};
