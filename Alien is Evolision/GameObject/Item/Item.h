...
class Item:public OtobeLib::TextureBase, OtobeLib::CollisionBase<OtobeLib::ITextureBase>
{
private:
	//取得済みフラグ
	bool m_getFlag;
public:
	//コンストラクタ
	Item(OtobeLib::ITextureBase* texture);
...
public:
	...
	//アイテムの取得済みフラグを設定する
	virtual void SetItemFlag(const bool& flag) { m_getFlag = flag; }
	...
	//アイテムの取得済みフラグを取得する
	virtual const bool& GetItemFlag()const { return m_getFlag; }
	...
	//自身の当たり判定を取得する
	virtual ICollisionBase<ITextureBase>* GetThisCollision() { return CollisionBase::GetThisCollision(); };

	//当たったかどうかのフラグを取得する
	virtual const bool& GetCheck()const override { return CollisionBase::GetCheck(); };
