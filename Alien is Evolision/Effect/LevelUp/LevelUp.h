...
class LevelUp :public OtobeLib::TextureBase,OtobeLib::Animation, State<LEVELUP>
{
public:
	//コンストラクタ
	LevelUp(OtobeLib::ITextureBase* texture);
...
public:
	//現在の状態を設定する
	void SetNowState(const LEVELUP& now)override { State::SetNowState(now); };

	//現在の状態を取得する
	const LEVELUP& GetNowState()const override { return State::GetNowState(); };

	//ひとつ前の状態を取得する
	const LEVELUP& GetPreState()const override { return State::GetPreState(); };
